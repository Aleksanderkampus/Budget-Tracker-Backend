...
public class
    ...
    where TBllEntity : class, IDomainIdentityId
    where TDalEntity : class, IDomainIdentityId
...
public class BaseEntityService<TBllEntity, TDalEntity, TRepository, TKey> : IEntityService<TBllEntity, TKey>
    where TBllEntity : class, IDomainIdentityId<TKey>
    where TDalEntity : class, IDomainIdentityId<TKey>
    ...
    where TKey : struct, IEquatable<TKey>
{
    ...
    public async Task<IEnumerable<TBllEntity>> AllAsync(Guid userId)
    {
        return (await Repository.AllAsync(userId)).Select(e => Mapper.Map(e));
    }
